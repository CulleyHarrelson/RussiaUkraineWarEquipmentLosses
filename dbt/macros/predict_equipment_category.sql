{% macro predict_equipment_category(column_name) %}
    CASE
        ...
        WHEN {{ column_name }} ~* 'A-\d+|Su-25|Warthog|Frogfoot' THEN 'Combat Aircraft'
        ...
        WHEN {{ column_name }} ~* 'EA-\d+|EF-\d+|Growler|Prowler' THEN 'Support Aircraft'
        ...
        WHEN {{ column_name }} ~* 'UH-\d+|CH-\d+|Mi-8|Chinook|Blackhawk|Helecopter' THEN 'Helicopter'
        ...
        /* Support Vehicle */
        WHEN {{ column_name }} ~* 'Fennek|BRDM|Fuchs' THEN 'Support Vehicle'
        WHEN {{ column_name }} ~* 'GAZ Tigr|International MaxxPro|Mastiff|Wolfhound|Husky|Kozak|Roshel Senator|MaxxPro' THEN 'Support Vehicle'
        WHEN {{ column_name }} ~* 'UAZ-469|HMMWV' THEN 'Support Vehicle'
        WHEN {{ column_name }} ~* 'Ural-\d+|KamAZ|GAZ-\d+|KrAZ|truck|ural' THEN 'Support Vehicle'
        WHEN {{ column_name }} ~* 'BREM-\d+|IMR-\d+|MTP-\d+' THEN 'Support Vehicle'
        WHEN {{ column_name }} ~* 'armoured\ recovery\ vehicle' THEN 'Support Vehicle'
        ...
        /* Naval Vessels */
        WHEN {{ column_name }} ~* 'CVN-|Admiral Kuznetsov|Queen Elizabeth' THEN 'Capital Ship'
        WHEN {{ column_name }} ~* 'DDG-|Type 052|Arleigh Burke' THEN 'Capital Ship'
        WHEN {{ column_name }} ~* 'SSN-|SSBN-|Akula|Typhoon|Astute' THEN 'Submarine'
        WHEN {{ column_name }} ~* 'FFG-|Type 054|FREMM' THEN 'Small Surface Combatant'
        WHEN {{ column_name }} ~* 'Corvette|Buyan-M|Visby' THEN 'Small Surface Combatant'
        WHEN {{ column_name }} ~* 'Patrol Boat|Dvora|CB90' THEN 'Small Surface Combatant'
        WHEN {{ column_name }} ~* 'LHD-|Mistral|Wasp' THEN 'Amphibious Warfare Ship'
        WHEN {{ column_name }} ~* 'Project \d+|Class' THEN 'Naval Vessel'

        /* Weapon Systems */
        WHEN {{ column_name }} ~* 'ZSU-23|Gepard|Tunguska' THEN 'Air Defense System'
        WHEN {{ column_name }} ~* 'S-300|S-400|Patriot|THAAD' THEN 'Air Defense System'
        ...
        WHEN {{ column_name }} ~* 'Krasukha|Murmansk-BN|EA-18G' THEN 'Electronic Warfare and Command System'
        WHEN {{ column_name }} ~* 'AN/SPY-1|AESA|Nebo|radar' THEN 'Electronic Warfare and Command System'
        WHEN {{ column_name }} ~* 'AWACS|E-3|A-50' THEN 'Electronic Warfare and Command System'
